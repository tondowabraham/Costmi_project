...
class Projects_model extends Crud_model {
    ...
    public function get_client_projects($client_id) {
        // Construct the SQL query with a WHERE condition for the client ID
        $sql = "SELECT DISTINCT title FROM {$this->table} WHERE client_id = ?";
        
        // Execute the query with the client ID as a parameter
        $query = $this->db->query($sql, [$client_id]);
        
        // Check for any query errors
        if (!$query) {
            // If there's an error, you can log it or return an empty array
            log_message('error', 'Error executing database query: ' . $this->db->error());
            return array();
        }
