...
class Estimates extends Security_Controller {
    ...
    function modal_form() {
        ...
        if (!$this->is_estimate_editable($id, $is_clone)) {
            app_redirect("forbidden");
        }
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $edit = "";
        if ($this->is_estimate_editable($data->id)) {
    ...
    function view($estimate_id = 0) {
        ...
        if ($estimate_id) {
            ...
            if ($view_data) {
                ...
                $view_data["is_estimate_editable"] = $this->is_estimate_editable($estimate_id);
    ...
    function discount_modal_form() {
        ...
        $this->validate_estimate_access($estimate_id);
        if (!$this->is_estimate_editable($estimate_id)) {
            app_redirect("forbidden");
        }
    ...
    function delete_item() {
        ...
        if (!$this->is_estimate_editable($item_info->estimate_id)) {
            app_redirect("forbidden");
        }
    ...
    private function _make_item_row($data) {
        ...
        if ($this->is_estimate_editable($data->estimate_id)) {
    ...
    private function is_estimate_editable($estimate_id = 0, $is_clone = 0) {
        if (!$estimate_id || $is_clone) {
            return true;
        }
