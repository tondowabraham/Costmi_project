...
class Contracts extends Security_Controller {
    ...
    function modal_form() {
        ...
        if (!$this->is_contract_editable($contract_id, $is_clone)) {
    ...
    function save() {
        ...
        if (!$this->is_contract_editable($id, $is_clone)) {
            app_redirect("forbidden");
        }
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $edit = "";
        if ($this->is_contract_editable($data->id)) {
    ...
    function view($contract_id = 0) {
        ...
        if ($contract_id) {
            ...
            if ($view_data) {
                ...
                $view_data["is_contract_editable"] = $this->is_contract_editable($contract_id);
    ...
    function discount_modal_form() {
        ...
        if (!$this->is_contract_editable($contract_id)) {
            app_redirect("forbidden");
        }
    ...
    function delete_item() {
        ...
        if (!$this->is_contract_editable($item_info->contract_id)) {
            app_redirect("forbidden");
        }
    ...
    private function _make_item_row($data) {
        ...
        if ($this->is_contract_editable($data->contract_id)) {
    ...
    private function is_contract_editable($contract_id = 0, $is_clone = 0) {
        if (!$contract_id || $is_clone) {
            return true;
        }
